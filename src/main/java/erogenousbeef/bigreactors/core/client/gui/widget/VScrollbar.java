...
public class VScrollbar {
    ...
    public void adjustPosition() {
        ...
        wholeArea = new Rectangle(xPosition, yPosition, (int) BRWidget.VSCROLL_THUMB_OFF.width, height);
        btnUp = new Rectangle(xPosition, yPosition, (int) BRWidget.UP_ARROW_OFF.width, (int) BRWidget.UP_ARROW_OFF.height);
        btnDown = new Rectangle(xPosition, yPosition + Math.max(0, height - (int) BRWidget.DOWN_ARROW_OFF.height), (int) BRWidget.DOWN_ARROW_OFF.width,
                (int) BRWidget.DOWN_ARROW_OFF.height);
        thumbArea = new Rectangle(xPosition, yPosition + btnUp.height, (int) BRWidget.VSCROLL_THUMB_OFF.width, Math.max(0, height
    ...
    public void drawScrollbar(int mouseX, int mouseY) {
        ...
        IWidgetIcon iconUp;
        if (pressedUp) {
            iconUp = hoverUp ? BRWidget.UP_ARROW_HOVER_ON : BRWidget.UP_ARROW_ON;
        } else {
            iconUp = hoverUp ? BRWidget.UP_ARROW_HOVER_OFF : BRWidget.UP_ARROW_OFF;
        }

        IWidgetIcon iconDown;
        if (pressedDown) {
            iconDown = hoverDown ? BRWidget.DOWN_ARROW_HOVER_ON : BRWidget.DOWN_ARROW_ON;
        } else {
            iconDown = hoverDown ? BRWidget.DOWN_ARROW_HOVER_OFF : BRWidget.DOWN_ARROW_OFF;
        }
        ...
        RenderUtil.bindTexture(BRWidget.TEXTURE);
        ...
        if (scrollMax > 0) {
            ...
            boolean hoverThumb = thumbArea.contains(mouseX, mouseY) && mouseY >= thumbPos && mouseY < thumbPos + (int) BRWidget.VSCROLL_THUMB_OFF.height;

            BRWidget iconThumb;
            if (pressedThumb) {
                iconThumb = BRWidget.VSCROLL_THUMB_HOVER_ON;
            } else {
                iconThumb = hoverThumb ? BRWidget.VSCROLL_THUMB_HOVER_OFF : BRWidget.VSCROLL_THUMB_OFF;
            }
    ...
    public boolean mouseClicked(int x, int y, int button) {
        if (button == 0) {
            if (scrollMax > 0 && thumbArea.contains(x, y)) {
                ...
                pressedDown = y >= thumbPos + (int) BRWidget.VSCROLL_THUMB_OFF.height;
    ...
    public boolean mouseClickMove(int x, int y, int button, long time) {
        if (pressedThumb) {
            int pos = y - (thumbArea.y + (int) BRWidget.VSCROLL_THUMB_OFF.height / 2);
            int len = thumbArea.height - (int) BRWidget.VSCROLL_THUMB_OFF.height;
            if (len > 0) {
    ...
    protected int getThumbPosition() {
        return thumbArea.y + (thumbArea.height - (int) BRWidget.VSCROLL_THUMB_OFF.height) * scrollPos / scrollMax;
    }
