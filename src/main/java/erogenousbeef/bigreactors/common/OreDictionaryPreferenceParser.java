...
import erogenousbeef.bigreactors.common.recipe.RecipeConfig;
import erogenousbeef.bigreactors.common.recipe.RecipeConfigParser;
...
public final class OreDictionaryPreferenceParser extends DefaultHandler {
    ...
    public static void loadConfig() {
        File coreFile = new File(BRConfig.configDirectory, CORE_FILE_NAME);

        String defaultVals = null;
        try {
            defaultVals = RecipeConfig.readRecipes(coreFile, CORE_FILE_NAME, true);
        } catch (IOException e) {
            BRLog.error("Could not load painter lists " + coreFile + " from EnderIO jar: " + e.getMessage());
            e.printStackTrace();
            return;
        }
        if(!coreFile.exists()) {
            BRLog.error("Could not load default lists from " + coreFile + " as the file does not exist.");
            return;
        }
        ...
        try {
            ...
            BRLog.error("Could not parse default lists from " + coreFile + ": " + e);
        }

        File userFile = new File(BRConfig.configDirectory, CUSTOM_FILE_NAME);
        ...
        try {
            ...
            BRLog.error("Could not load user defined painter lists from file: " + CUSTOM_FILE_NAME);
            e.printStackTrace();
        }
    }
        ...
        if(ELEMENT_PREF.equals(localName)) {
            if(oreDictName != null && prefStack != null) {
                ...
                if(matched) {
                    ...
                    BRLog.warning("OreDictionaryPreferenceParser: Attempted to register " + prefStack + " as the preffered output for " + oreDictName
