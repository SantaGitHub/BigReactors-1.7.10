...
import erogenousbeef.bigreactors.core.util.NetworkUtil;
import io.netty.buffer.ByteBuf;
import net.minecraft.entity.player.EntityPlayer;
import net.minecraft.nbt.NBTTagCompound;

public class PacketTanks extends MessageTileEntity<TileEntityLiquidizer> implements IMessageHandler<PacketTanks, IMessage> {
    ...
    public PacketTanks(TileEntityLiquidizer tile) {
        super(tile);
    ...
    public IMessage onMessage(PacketTanks message, MessageContext ctx) {
        EntityPlayer player = ctx.side == Side.SERVER ? ctx.getServerHandler().playerEntity : BRLoader.proxy.getClientPlayer();
        TileEntityLiquidizer tile = message.getTileEntity(player.worldObj);
        if(tile == null) {
            return null;
        }
