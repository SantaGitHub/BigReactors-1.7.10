...
import erogenousbeef.bigreactors.common.OreDictionaryPreferences;
import erogenousbeef.bigreactors.common.recipe.RecipeConfig.RecipeGroup;
import erogenousbeef.bigreactors.common.recipe.RecipeConfig.RecipeElement;
import erogenousbeef.bigreactors.utils.OreDictionaryHelper;
...
public class RecipeConfigParser extends DefaultHandler {
    ...
    public void warning(SAXParseException e) throws SAXException {
        BRLog.warning("Warning parsing SAG Mill config file: " + e.getMessage());
    }

    @Override
    public void error(SAXParseException e) throws SAXException {
        BRLog.error("Error parsing SAG Mill config file: " + e.getMessage());
        e.printStackTrace();
    }
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        ...
        if(ELEMENT_RECIPE_GROUP.equals(localName)) {
            ...
            if(debug) {
                BRLog.debug(LP + "Closing recipe group");
            }
        ...
        if(ELEMENT_RECIPE.equals(localName)) {
            if(debug) {
                BRLog.debug(LP + "Closing recipe");
            }
            if(recipe != null && recipeGroup != null) {
                recipeGroup.addRecipe(recipe);
            } else {
                BRLog.warning(LP + "Could not add recipe " + recipe + " to group " + recipeGroup);
            }
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
        ...
        if(debug) {
            ...
            BRLog.debug(LP + "RecipeConfigParser.startElement: localName:" + localName + " attrs:" + sb);
        }

        if(isElementRoot(localName)) {
            if(root != null) {
                BRLog.warning(LP + "Multiple root elements found.");
            } else {
        ...
        if(root == null) {
            BRLog.warning(LP + " Root element not specified before element " + localName + ".");
            root = new RecipeConfig();
        }
        ...
        if(ELEMENT_RECIPE_GROUP.equals(localName)) {
            if(recipeGroup != null) {
                BRLog.warning(LP + "Recipe group " + recipeGroup.getName() + " not closed before encountering a new recipe group.");
            }
            ...
            if(!recipeGroup.isNameValid()) {
                BRLog.warning(LP + "A recipe group was found with an invalid name: " + attributes.getValue(AT_NAME));
                recipeGroup = null;
            }
            return;
        }
        ...
        if(ELEMENT_RECIPE.equals(localName)) {
            if(recipeGroup == null) {
                BRLog.warning(LP + "A recipe was found outside of a recipe groups tags.");
                return;
            }
            if(recipe != null) {
                BRLog.warning(LP + "A new recipe was started before the recipe was closed.");
            }
            String name = getStringValue(AT_NAME, attributes, null);
            if(name == null) {
                BRLog.warning(LP + "An unnamed recipe was found.");
                return;
            }
        ...
        if(ELEMENT_OUTPUT.equals(localName)) {
            if(inputTagOpen) {
                BRLog.warning(LP + "<output> encounterd before <input> closed.");
                inputTagOpen = false;
            }
            if(outputTagOpen) {
                BRLog.warning(LP + "<output> encounterd before previous <output> closed.");
            }
            outputTagOpen = true;
            return;
        }
        ...
        if(ELEMENT_INPUT.equals(localName)) {
            if(outputTagOpen) {
                BRLog.warning(LP + "<input> encounterd before <output> closed.");
                outputTagOpen = false;
            }
            if(inputTagOpen) {
                BRLog.warning(LP + "<input> encounterd before previous <input> closed.");
            }
            inputTagOpen = true;
            return;
        }
        ...
        if(ELEMENT_ITEM_STACK.equals(localName) || isFluidStack) {
            if(!inputTagOpen && !outputTagOpen) {
                BRLog.warning(LP + "Encounterd an item stack outside of either an <input> or <output> tag.");
                return;
            }
            if(inputTagOpen && outputTagOpen) {
                BRLog.warning(LP + "Encounterd an item stack within both an <input> and <output> tag.");
                return;
            }
    ...
    private RecipeInput getFluidStack(Attributes attributes) {
        ...
        if(fluid == null) {
            BRLog.warning("When parsing recipes could not find fluid with name: " + name);
            return null;
        }
    ...
    public static RecipeInput getItemStack(Attributes attributes) {
        ...
        if(oreDict != null) {
            if(!OreDictionaryHelper.isRegistered(oreDict)) {
                BRLog.debug(LP + "Could not find an entry in the ore dictionary for " + oreDict);
                return null;
            }
            ItemStack stack = OreDictionaryPreferences.instance.getPreferred(oreDict);
            if(stack == null) {
                BRLog.debug(LP + "Could not find a prefered item  in the ore dictionary for " + oreDict);
                return null;
            }
            stack = stack.copy();
        ...
        if(res == null) {
            BRLog.debug("Could not create an item stack from the attributes " + toString(attributes));
            return null;
        }
    ...
    public static int getIntValue(String qName, Attributes attributes, int def) {
        try {
            ...
            BRLog.warning(LP + "Could not parse a valid int for attribute " + qName + " with value " + getStringValue(qName, attributes, null));
            return def;
        }
    }
    ...
    public static float getFloatValue(String qName, Attributes attributes, float def) {
        try {
            ...
            BRLog.warning(LP + "Could not parse a valid float for attribute " + qName + " with value " + getStringValue(qName, attributes, null));
            return def;
        }
    }
