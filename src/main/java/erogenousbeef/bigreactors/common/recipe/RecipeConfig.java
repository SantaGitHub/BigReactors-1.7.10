...
public class RecipeConfig {
    ...
    public static RecipeConfig loadRecipeConfig(String coreFileName, String customFileName, CustomTagHandler customHandler) {
        ...
        try {
            ...
            BRLog.error("Could not load default recipes file " + coreFile + " from EnderIO jar: " + e.getMessage());
            e.printStackTrace();
            return null;
        }

        if(!coreFile.exists()) {
            BRLog.error("Could not load default recipes from " + coreFile + " as the file does not exist.");
            return null;
        }
        ...
        try {
            ...
            BRLog.error("Error parsing " + coreFileName);
            return null;
        }

        File userFile = new File(BRConfig.configDirectory, customFileName);
        ...
        try {
            ...
            if(userConfigStr == null || userConfigStr.trim().length() == 0) {
                BRLog.error("Empty user config file: " + userFile.getAbsolutePath());
            } else {
                RecipeConfig userConfig = RecipeConfigParser.parse(userConfigStr, customHandler);
                config.merge(userConfig);
            }
        } catch (Exception e) {
            BRLog.error("Could not load user defined recipes from file: " + customFileName);
            e.printStackTrace();
        }
        return config;
    }
    ...
    public static String readRecipes(File copyTo, String fileName, boolean replaceIfExists) throws IOException {
        ...
        InputStream in = RecipeConfig.class.getResourceAsStream("/assets/bigreactors/config/" + fileName);
        if(in == null) {
            BRLog.error("Could load default AlloySmelter recipes.");
            throw new IOException("Could not resource /assets/bigreactors/config/" + fileName + " form classpath. ");
        }
        ...
        for (RecipeGroup group : userConfig.getRecipeGroups().values()) {
            if(!group.enabled) {
                if(recipeGroups.remove(group.name) != null) {
                    BRLog.info("Disabled core recipe group " + group.name + " due to user config.");
                }
            } else {
                RecipeGroup modifyGroup = recipeGroups.get(group.name);
                if(modifyGroup == null) {
                    BRLog.info("Added user defined recipe group " + group.name);
                ...
                for (RecipeElement recipe : group.recipes.values()) {
                    if(recipe.isValid()) {
                        if(modifyGroup.recipes.containsKey(recipe.name)) {
                            BRLog.info("Replacing core recipe " + recipe.name + "  with user defined recipe.");
                        } else {
                            BRLog.info("Added user defined recipe " + recipe.name);
                        }
                        modifyGroup.addRecipe(recipe);
                    } else {
                        BRLog.info("Removed recipe " + recipe.name + " due to user config.");
