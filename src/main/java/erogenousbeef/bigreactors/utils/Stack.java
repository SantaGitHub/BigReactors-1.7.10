...
public class Stack {
    ...
    public static Stack parseStackString(String stackString, int missingMetaValue) {
        ...
        if (parts.length != 2 && parts.length != 3) {
            BRLog.warning("Stack string (" + stackString + ") isn't formatted properly. Suitable formats are <modId>:<name>, <modId>:<name>:<meta> or <modId>:<name>:*, e.g. IC2:blockWall:*");
            return null;
        }
        ...
        if (parts.length == 2) {
            ...
            try {
                ...
                BRLog.warning("ItemStack string (" + stackString + ") has improperly formatted metadata. Suitable metadata are integer values or *.");
                return null;
            }
        }
    ...
    public Item getItem() {
        ...
        if (item == null) {
            BRLog.warning("Failed to find item for (" + name + ") in the Forge registry.");
        }
        return item;
    }
    ...
    public Block getBlock() {
        ...
        if (block == null) {
            BRLog.warning("Failed to find block for (" + name + ") in the Forge registry.");
        }
        return block;
    }
