...
import erogenousbeef.bigreactors.common.block.BlockLiquidizer;
...
import erogenousbeef.bigreactors.core.client.render.BoundingBox;
...
import erogenousbeef.bigreactors.core.util.IBlockAccessWrapper;
import erogenousbeef.bigreactors.core.util.vecmath.*;
import erogenousbeef.bigreactors.net.CommonPacketHandler;
import erogenousbeef.bigreactors.utils.RenderPassHelper;
...
public class IoConfigRenderer {
        ...
        if(!Mouse.getEventButtonState() && camera.isValid() && elapsed > 500) {
            if(Mouse.getEventButton() == 1) {
                if(selection != null) {
                    ...
                    CommonPacketHandler.INSTANCE.sendToServer(new PacketIoMode(selection.config,selection.face));
                }
    ...
    private void renderSelection() {
        ...
        IIcon icon = BigReactors.blockLiquidizer.selectedFaceIcon;
    ...
    private void renderOverlay(int mx, int my) {
        ...
        if(mx >= x && mx <= x + IconBR.IO_WHATSIT.width &&
                my >= y && my <= y + IconBR.IO_WHATSIT.height) {
            RenderUtil.renderQuad2D(x, y, 0, IconBR.IO_WHATSIT.width, IconBR.IO_WHATSIT.height, new Vector4f(0.4f,0.4f,0.4f,0.6f));
        ...
        GL11.glColor3f(1, 1, 1);
        IconBR.map.render(IconBR.IO_WHATSIT, x, y, true);
        ...
        if(selection != null) {
            ...
            if(mode == IoMode.PULL) {
                ...
                ioIcon = IconBR.OUTPUT;
            } else if(mode == IoMode.PUSH_PULL) {
                ioIcon = IconBR.INPUT_OUTPUT;
            } else if(mode == IoMode.DISABLED) {
                ioIcon = IconBR.DISABLED;
            }
